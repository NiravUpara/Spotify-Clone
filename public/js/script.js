    ...
    try {
        ...
        Array.from(document.querySelectorAll(".songList li")).forEach((e, index) => {
            e.addEventListener("click", () => {
                ...
                if (isSameSong && !currentSong.paused) {
                    currentSong.pause();
                    document.querySelector("#play").src = "/img/play.svg";
                    updateSongListIcons(-1); // Reset all to play icon
                } else {
...
function updateSongListIcons(activeIndex) {
    function updateSongListIcons(activeIndex) {
        Array.from(document.querySelectorAll(".songList li")).forEach((e, index) => {
            ...
            if (index === activeIndex && !currentSong.paused) {
                icon.src = "/img/pause.svg";
            } else {
                icon.src = "/img/play.svg";
            }

            e.classList.toggle("active-song", index === activeIndex && !currentSong.paused);
        });
    }
